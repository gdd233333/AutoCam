...
**软件 SAT（Cycle cam `4` 或 `hal.multi_lens`）**：不打开硬件 `4`。按用户变焦 0.61–10（相对主摄 23 mm）在物理 `2` / `0` / `3` 之间切。远跳一次到位。预览单流、关镜不等 onClosed，忙则 20 ms 重试。切点默认 UW→main `0.95` / main→UW `0.72`，main→tele `5.10` / tele→main `4.95`（长焦传感器不能小于 1.0x / 5.22x，回切必须贴近 5x 否则 5→3.5 画面不变）。每颗镜头的 `CONTROL_ZOOM_RATIO` 从该传感器的 1.0 起算。直接开 `2`/`3` 仍可单独用 UW/Tele。
