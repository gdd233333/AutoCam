...
from ml.students.letterbox import box_xyxy_to_cxcywh_letterbox, letterbox_pil
...
def process_record(rec: dict) -> tuple | None:
    ...
    if rec.get("box_xyxy"):
        ...
        if area < 0.015:
            ...
            obj = 1.0
    if rec.get("label_i") is not None:
    ...
    logit[label_i] = 4.0
    rgb = np.transpose(canvas_u8, (2, 0, 1))
    return rgb, box, label_i, obj, logit
...
def run(
    ...
    with index_path.open(encoding="utf-8") as f:
        for line in f:
            ...
            if line:
                rec = json.loads(line)
                if not use_index_label:
                    rec.pop("label_i", None)
                records.append(rec)
    if limit > 0:
    ...
    print(f"n={len(records)} workers={workers} shards -> {out_dir} size={shard_size}")
    rgbs, boxes, labels, objs, logits_all = [], [], [], [], []
    shard_i = 0
    kept = 0
    done = 0

    def consume(item: tuple | None) -> None:
        nonlocal shard_i, kept, done
        done += 1
        if item is None:
            return
        rgb, box, label_i, obj, logit = item
        rgbs.append(rgb)
        boxes.append(box)
        ...
        kept += 1
        if done % 500 == 0:
            print(f"  {done}/{len(records)} kept={kept}")
    ...
    if workers == 1:
        for rec in records:
            consume(process_record(rec))
    else:
        with ProcessPoolExecutor(max_workers=workers) as pool:
            for item in pool.map(process_record, records, chunksize=16):
                consume(item)
    _flush_shard(out_dir, shard_i, rgbs, boxes, labels, objs, logits_all)
...
def main() -> None:
    ...
    run(args.index, args.out, args.limit, args.device, args.use_index_label, args.shard_size, args.workers)
